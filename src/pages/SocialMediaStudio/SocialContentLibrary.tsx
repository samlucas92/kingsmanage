...
export default function SocialContentLibrary() {
	...
	const [deleteTarget, setDeleteTarget] = useState<SocialPublication | null>(null);
	...
	async function deleteContent() {
		...
		try {
			...
			setError("");
			await socialPublicationsApi.delete(deleteTarget.id);
			setItems((current) => current.filter((item) => item.id !== deleteTarget.id));
			setDeleteTarget(null);
		} catch (deleteError) {
			setError(deleteError instanceof Error ? deleteError.message : "The content could not be deleted.");
		} finally {
			setBusyId("");
		}
	}
	...
	return <div className="space-y-5">
		...
		{isLoading ? <div className="py-16 text-center text-sm font-semibold text-slate-500">Loading content…</div> : visible.length === 0 ? <div className="surface-card px-5 py-16 text-center"><p className="font-black text-slate-900">No content in this view</p><p className="mt-1 text-sm text-slate-500">Create a graphic and save it to start your library.</p></div> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{visible.map((item) => <ContentCard key={item.id} item={item} busy={busyId === item.id} onUpdate={update} onDownload={download} onDelete={setDeleteTarget} />)}</div>}
		<ConfirmationModal isOpen={deleteTarget !== null} title="Delete unsent content?" message="This removes the saved post and its image from Yepset. It cannot be undone. Content already sent to Meta cannot be deleted here." confirmText="Delete content" isBusy={busyId === deleteTarget?.id} variant="danger" onCancel={() => setDeleteTarget(null)} onConfirm={deleteContent} />
	</div>;
}

function ContentCard({ item, busy, onUpdate, onDownload, onDelete }: { item: SocialPublication; busy: boolean; onUpdate: (item: SocialPublication, action: "publish" | "draft" | "retry" | "cancel") => void; onDownload: (item: SocialPublication) => void; onDelete: (item: SocialPublication) => void }) {
	const hasFacebook = item.deliveries.some((delivery) => delivery.platform === "Facebook");
	const facebookPageId = item.deliveries.find((delivery) => delivery.platform === "Facebook")?.destinationId;
	const facebookDraftId = item.deliveries.find((delivery) => delivery.platform === "Facebook")?.providerPostId;
	const metaDraftUrl = facebookPageId ? `https://business.facebook.com/latest/posts/draft_posts?asset_id=${encodeURIComponent(facebookPageId)}` : "https://business.facebook.com/";
	const canDelete = ["Draft", "Failed", "Cancelled"].includes(item.status) && item.deliveries.every((delivery) => !delivery.providerPostId);
	return <article className="surface-card overflow-hidden">
		...
		<div className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="truncate font-black text-slate-950">{item.title}</h2><p className="mt-1 text-xs font-semibold text-slate-500">{item.deliveries.map((delivery) => delivery.platform).join(" + ")} · {new Date(item.updatedAt).toLocaleString()}</p></div><StatusBadge status={item.status} /></div>
			...
			<div className="mt-4 flex flex-wrap gap-2"><Link to={`/social-media?contentId=${item.id}`} className="btn-secondary px-3 py-1.5 text-xs">Use as new</Link><button type="button" onClick={() => void onDownload(item)} disabled={!item.fileId || busy} className="btn-secondary px-3 py-1.5 text-xs">Download</button>{item.status === "MetaDraft" && <><a href={metaDraftUrl} target="_blank" rel="noreferrer" className="btn-primary px-3 py-1.5 text-xs">Open Facebook drafts ↗</a>{facebookDraftId && <button type="button" onClick={() => void navigator.clipboard.writeText(facebookDraftId)} className="btn-secondary px-3 py-1.5 text-xs" title={facebookDraftId}>Copy Meta ID</button>}</>}{item.status === "Draft" && <><button type="button" onClick={() => onUpdate(item, "publish")} disabled={busy} className="btn-primary px-3 py-1.5 text-xs">Publish now</button>{hasFacebook && <button type="button" onClick={() => onUpdate(item, "draft")} disabled={busy} className="btn-secondary px-3 py-1.5 text-xs">Send Facebook draft</button>}</>}{["Failed", "PartiallyPublished"].includes(item.status) && <button type="button" onClick={() => onUpdate(item, "retry")} disabled={busy} className="btn-secondary px-3 py-1.5 text-xs">Retry</button>}{["Draft", "Scheduled"].includes(item.status) && <button type="button" onClick={() => onUpdate(item, "cancel")} disabled={busy} className="px-3 py-1.5 text-xs font-black text-rose-700">Cancel</button>}{canDelete && <button type="button" onClick={() => onDelete(item)} disabled={busy} className="px-3 py-1.5 text-xs font-black text-rose-700">Delete</button>}</div>
		</div>
	</article>;
}
