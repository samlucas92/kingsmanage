...
function LeagueRuleModal({ rule, profiles, onClose, onSave }: { rule: LeagueRule | null; profiles: ReturnType<typeof useClubTeamStore.getState>["profiles"]; onClose: () => void; onSave: (values: SaveLeagueRule) => Promise<void> }) {
	...
	const [higherCompetitions, setHigherCompetitions] = useState((rule?.higherTeamCompetitions ?? []).join(", "));
	const [restrictedCompetitions, setRestrictedCompetitions] = useState((rule?.restrictedTeamCompetitions ?? []).join(", "));
	...
	async function submit(event: FormEvent) {
		...
		try { await onSave({ ...values, maxPlayers: values.ruleType === "CupTied" ? null : values.maxPlayers, higherTeamCompetitions: splitCompetitions(higherCompetitions), restrictedTeamCompetitions: splitCompetitions(restrictedCompetitions) }); } finally { setSaving(false); }
	}
	return <div className="fixed inset-0 z-50 grid place-items-center bg-yepset-950/55 p-4 backdrop-blur-sm"><form onSubmit={(event) => void submit(event)} className="max-h-[92vh] w-full max-w-2xl space-y-4 overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
		...
		<div className="grid gap-4 sm:grid-cols-2"><Field label="Higher team"><select value={values.higherTeamId} onChange={(event) => update("higherTeamId", event.target.value)} className="input-field">{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.displayName}</option>)}</select></Field><Field label="Restricted team"><select value={values.restrictedTeamId} onChange={(event) => update("restrictedTeamId", event.target.value)} className="input-field">{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.displayName}</option>)}</select></Field></div>
		...
		<Field label="Qualifying higher-team competitions"><input value={higherCompetitions} onChange={(event) => setHigherCompetitions(event.target.value)} className="input-field" placeholder="League, League Cup" /><span className="mt-1 block text-xs text-slate-500">Comma-separated. Leave blank for all competitions.</span></Field>
		<Field label="Matches where this rule applies"><input value={restrictedCompetitions} onChange={(event) => setRestrictedCompetitions(event.target.value)} className="input-field" placeholder="League Cup" /><span className="mt-1 block text-xs text-slate-500">Comma-separated. Leave blank for all competitions.</span></Field>
...
function splitCompetitions(value: string) { return [...new Set(value.split(",").map((item) => item.trim()).filter(Boolean))]; }
